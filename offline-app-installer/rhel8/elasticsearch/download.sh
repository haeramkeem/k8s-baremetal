...
# Download values
curl -L https://raw.githubusercontent.com/haeramkeem/yammy/main/helm-values/elastic/elasticsearch/values.yaml -o $WORKDIR/values/elasticsearch.yaml
helm template $WORKDIR/charts/elasticsearch.tgz -f $WORKDIR/values/elasticsearch.yaml \
...
curl -L $INSTALL_SH_PREFIX/elasticsearch/src/install.sh -o $WORKDIR/install.sh
