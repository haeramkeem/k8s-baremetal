...
helm pull bitnami/postgresql-ha --version $VERSION
mv -v postgresql-ha-$VERSION.tgz $WORKDIR/charts/postgresql-ha.tgz

# download values
curl -L -o $WORKDIR/values/postgresql-ha.values.yaml \
    https://github.com/haeramkeem/yammy/blob/main/helm/values/bitnami/postgresql-ha/harbor-ext-ha-db.values.yaml
...
curl -Lo $WORKDIR/install.sh \
    https://raw.githubusercontent.com/haeramkeem/k8s-iac/main/offline-app-installer/HA-PostgreSQL/focal64/src/install.sh
